...
int filterbankh5_open(filterbankh5_context_t * p_fbh5_ctx, filterbank_header_t * p_fb_hdr, unsigned int Nd, char * output_path, int debug_callback) {
    ...
    if (H5Zfilter_avail(FILTER_ID_BITSHUFFLE) <= 0)
        filterbankh5_warning(__FILE__, __LINE__, "fbhf_open: Plugin bitshuffle is NOT available; data will not be compressed\n");
    else {
    ...
    if((p_fb_hdr->nbits % 8 != 0) || (p_fb_hdr->nbits < 8) || (p_fb_hdr->nbits > 64)) {
        sprintf(wstr, "filterbankh5_open: nbits must be in [8, 16, 32, 64] but I saw %d", p_fb_hdr->nifs);
        filterbankh5_error(__FILE__, __LINE__, wstr);
        return 1;
    }
    if((p_fb_hdr->nifs < 1) || (p_fb_hdr->nifs > 4)) {
        sprintf(wstr, "filterbankh5_open: nifs must be in [1, 2, 3, 4] but I saw %d", p_fb_hdr->nifs);
        filterbankh5_error(__FILE__, __LINE__, wstr);
        return 1;
    }
    if(p_fb_hdr->nfpc < 1) {
        sprintf(wstr, "filterbankh5_open: nfpc must be > 0 but I saw %d", p_fb_hdr->nfpc);
        filterbankh5_error(__FILE__, __LINE__, wstr);
        return 1;
    }
    if(p_fb_hdr->nchans < 1) {
        sprintf(wstr, "filterbankh5_open: nchans must be > 0 but I saw %d", p_fb_hdr->nchans);
        filterbankh5_error(__FILE__, __LINE__, wstr);
        return 1;
    }
     ...
     */
    memset(p_fbh5_ctx, 0, sizeof(filterbankh5_context_t));
     ...
     */
    filterbankh5_set_str_attr(p_fbh5_ctx->file_id, 
                      "CLASS", 
                      FILTERBANK_CLASS, 
                      debug_callback);
    filterbankh5_set_str_attr(p_fbh5_ctx->file_id, 
    ...
    if(p_fbh5_ctx->dataspace_id < 0) {
        filterbankh5_error(__FILE__, __LINE__, "fbh5_open: H5Screate_simple FAILED");
        return 1;
    }
    ...
    if(dcpl < 0) {
        filterbankh5_error(__FILE__, __LINE__, "fbh5_open: H5Pcreate/dcpl FAILED");
        return 1;
    }
    ...
    if(USE_BLIMPY == 1)
        filterbankh5_blimpy_chunking(p_fb_hdr, &cdims[0]);
    else {
        cdims[0] = Nd;
        cdims[1] = 1;
    ...
    if(status != 0) {
        filterbankh5_error(__FILE__, __LINE__, "fbh5_open: H5Pset_chunk FAILED");
        return 1;
    }
    ...
    if(bitshuffle_available) {
        ...
        if(status < 0)
            filterbankh5_warning(__FILE__, __LINE__, "fbh5_open: H5Pset_filter FAILED; data will not be compressed");
    }
    ...
    if(p_fbh5_ctx->dataset_id < 0) {
        filterbankh5_error(__FILE__, __LINE__, "fbh5_open: H5Dcreate FAILED");
        return 1;
    }
    ...
    if(CACHING_TYPE != 2) { // not default caching
        ...
        if(fapl < 0)
            filterbankh5_warning(__FILE__, __LINE__, "fbh5_open: H5Fget_access_plist FAILED; using default caching");
        else {
            ...
            if(status < 0)
                filterbankh5_warning(__FILE__, __LINE__, "fbh5_open: H5Pset_cache FAILED; using default caching");
        }
    }
    ...
    if(status != 0)
        filterbankh5_warning(__FILE__, __LINE__, "fbh5_open: H5Pclose/dcpl FAILED\n");
    ...
    if(CACHING_TYPE != 2) {
        ...
        if(status != 0)
            filterbankh5_warning(__FILE__, __LINE__, "fbh5_open: H5Pclose/fapl FAILED\n");
    }

    /*
     * Write dataset metadata attributes.
     */
    filterbankh5_write_metadata(p_fbh5_ctx->dataset_id, // Dataset handle
                        p_fb_hdr,               // Metadata (SIGPROC header)
                        debug_callback);        // Tracing flag
    if(debug_callback)
        filterbankh5_info("fbh5_open: Dataset metadata stored; done.\n");
    ...
    p_fbh5_ctx->active = 1;
    if(debug_callback)
        filterbankh5_show_context("fbh5_open", p_fbh5_ctx);
    return 0;
