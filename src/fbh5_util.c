...
***/
void filterbankh5_set_str_attr(hid_t file_or_dataset_id, char * tag, char * p_value, int debug_callback) {
    herr_t status;
    hid_t id_scalar, atype, id_attr;
    char warning[256];
   
    if(debug_callback)
        filterbankh5_info("fbh5_set_str_attr: %s = %s\n", tag, p_value);
    ...
    if(id_attr < 0) {
        ...
        filterbankh5_warning(__FILE__, __LINE__, warning);
        return;
    }
...
void filterbankh5_set_dataset_double_attr(hid_t dataset_id, char * tag, double * p_value, int debug_callback) {
    ...
    if(debug_callback)
        filterbankh5_info("fbh5_set_dataset_double_attr: %s = %f\n", tag, *p_value);
    id_scalar  = H5Screate(H5S_SCALAR);
    if(id_scalar < 0) {
...
void filterbankh5_set_dataset_int_attr(hid_t dataset_id, char * tag, int * p_value, int debug_callback) {
    ...
    if(debug_callback)
        filterbankh5_info("fbh5_set_dataset_int_attr: %s = %d\n", tag, *p_value);
    id_scalar  = H5Screate(H5S_SCALAR);
    if(id_scalar < 0) {
...
***/
void filterbankh5_write_metadata(hid_t dataset_id, filterbankh5_header_t *p_md, int debug_callback) {
    filterbankh5_set_dataset_int_attr(dataset_id, "machine_id", &(p_md->machine_id), debug_callback);
    filterbankh5_set_dataset_int_attr(dataset_id, "telescope_id", &(p_md->telescope_id), debug_callback);
    filterbankh5_set_dataset_int_attr(dataset_id, "data_type", &(p_md->data_type), debug_callback);
    filterbankh5_set_dataset_int_attr(dataset_id, "nchans", &(p_md->nchans), debug_callback);
    filterbankh5_set_dataset_int_attr(dataset_id, "nfpc", &(p_md->nfpc), debug_callback);
    filterbankh5_set_dataset_int_attr(dataset_id, "nbeams", &(p_md->nbeams), debug_callback);
    filterbankh5_set_dataset_int_attr(dataset_id, "ibeam", &(p_md->ibeam), debug_callback);
    filterbankh5_set_dataset_int_attr(dataset_id, "nbits", &(p_md->nbits), debug_callback);
    filterbankh5_set_dataset_int_attr(dataset_id, "nifs", &(p_md->nifs), debug_callback);
    filterbankh5_set_dataset_double_attr(dataset_id, "src_raj", &(p_md->src_raj), debug_callback);
    filterbankh5_set_dataset_double_attr(dataset_id, "src_dej", &(p_md->src_dej), debug_callback);
    filterbankh5_set_dataset_double_attr(dataset_id, "az_start", &(p_md->az_start), debug_callback);
    filterbankh5_set_dataset_double_attr(dataset_id, "za_start", &(p_md->za_start), debug_callback);
    filterbankh5_set_dataset_double_attr(dataset_id, "fch1", &(p_md->fch1), debug_callback);
    filterbankh5_set_dataset_double_attr(dataset_id, "foff", &(p_md->foff), debug_callback);
    filterbankh5_set_dataset_double_attr(dataset_id, "tstart", &(p_md->tstart), debug_callback);
    filterbankh5_set_dataset_double_attr(dataset_id, "tsamp", &(p_md->tsamp), debug_callback);
    filterbankh5_set_str_attr(dataset_id, "source_name", &(p_md->source_name[0]), debug_callback);
    filterbankh5_set_str_attr(dataset_id, "rawdatafile", &(p_md->rawdatafile[0]), debug_callback);
}
...
void filterbankh5_set_ds_label(filterbankh5_context_t * p_fbh5_ctx, char * label, int dims_index, int debug_callback) {
    ...
    if(debug_callback)
        filterbankh5_info("fbh5_set_ds_label: label = %s, dims_index = %d\n", label, dims_index);
    ...
    if(status < 0) {
        ...
        filterbankh5_warning(__FILE__, __LINE__, wstr);
    }
}
...
void filterbankh5_show_context(char * caller, filterbankh5_context_t * p_fbh5_ctx) {
    if(p_fbh5_ctx == NULL) {
        filterbankh5_info("*** fbh5_show_context: p_fbh5_ctx = NULL !!!");
        return;
    }
    filterbankh5_info("fbh5_show_context(%s): active = %d\n", caller, p_fbh5_ctx->active);
    filterbankh5_info("fbh5_show_context(%s): elem_size = %d\n", caller, p_fbh5_ctx->elem_size);
    filterbankh5_info("fbh5_show_context(%s): tint_size = %ld\n", caller, p_fbh5_ctx->tint_size);
    filterbankh5_info("fbh5_show_context(%s): offset_dims = (%lld, %lld, %lld)\n",
           caller, p_fbh5_ctx->offset_dims[0], p_fbh5_ctx->offset_dims[1], p_fbh5_ctx->offset_dims[2]);
    filterbankh5_info("fbh5_show_context(%s): filesz_dims = (%lld, %lld, %lld)\n",
           caller, p_fbh5_ctx->filesz_dims[0], p_fbh5_ctx->filesz_dims[1], p_fbh5_ctx->filesz_dims[2]);
    filterbankh5_info("fbh5_show_context(%s): byte_count = %ld\n", caller, p_fbh5_ctx->byte_count);
    filterbankh5_info("fbh5_show_context(%s): dump_count = %ld\n", caller, p_fbh5_ctx->dump_count);
}
